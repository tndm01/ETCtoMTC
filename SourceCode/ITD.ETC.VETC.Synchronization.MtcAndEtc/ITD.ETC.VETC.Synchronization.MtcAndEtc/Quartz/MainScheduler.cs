        ...
        public void JobManagerment()
        {
            try
            {
                ...
                if(commuterJob != null && commuterJob.IntervalSynchrozation > 0 && commuterJob.IsRun)
                {
                    ...
                    .WithSimpleSchedule(s => s.WithIntervalInMinutes(commuterJob.IntervalSynchrozation).RepeatForever())
                    .StartNow()
                    .Build();
                ...
                if (employeeJob != null && employeeJob.IntervalSynchrozation > 0 && employeeJob.IsRun)
                {
                    ...
                    ITrigger trigEmployeeData = TriggerBuilder.Create()
                        .WithSimpleSchedule(s => s.WithIntervalInMinutes(employeeJob.IntervalSynchrozation).RepeatForever())
                        .StartNow()
                        .Build();
                ...
                var vehicleJob = _config.MtcJobList.FirstOrDefault(j => j.JobName == Resources.VehiclePlateDataJobName);
                if(vehicleJob != null && vehicleJob.IntervalSynchrozation > 0 && vehicleJob.IsRun)
                {
                    ...
                    .WithSimpleSchedule(s => s.WithIntervalInMinutes(vehicleJob.IntervalSynchrozation).RepeatForever())
                    .StartNow()
                    .Build();
                ...
                if (specialJob != null && specialJob.IntervalSynchrozation > 0 && specialJob.IsRun)
                {
                    ...
                    ITrigger trigSpecicalTransaction = TriggerBuilder.Create()
                        .WithSimpleSchedule(s => s.WithIntervalInMinutes(specialJob.IntervalSynchrozation).RepeatForever())
                        .StartNow()
                        .Build();
                    scheduler.ScheduleJob(jobSpecicalTransaction, trigSpecicalTransaction);
                }

                ////luồng SpecicalBTCTransactionData ETC-MTC
                var specialBTCJob = _config.EtcJobList.FirstOrDefault(j => j.JobName == Resources.SpecialBTCTransactionDataJobName);
                if (specialBTCJob != null && specialBTCJob.IntervalSynchrozation > 0 && specialBTCJob.IsRun)
                {
                    IJobDetail jobSpecicalBTCTransaction = JobBuilder.Create<SpecialBTCTransactionETCtoMTC>()
                    .WithIdentity("SpecialBTCTransactionData", "ETC->MTC")
                    .Build();
                    ITrigger trigSpecicalBTCTransaction = TriggerBuilder.Create()
                        .WithSimpleSchedule(s => s.WithIntervalInMinutes(specialBTCJob.IntervalSynchrozation).RepeatForever())
                        .StartNow()
                        .Build();
                    scheduler.ScheduleJob(jobSpecicalBTCTransaction, trigSpecicalBTCTransaction);
                }
                ...
                //luồng Image ETC-MTC
                var imageJob = _config.EtcJobList.FirstOrDefault(j => j.JobName == Resources.ImageDataJobName);
                if (imageJob != null && imageJob.IntervalSynchrozation > 0 && imageJob.IsRun)
                {
                    IJobDetail jobImage = JobBuilder.Create<LaneImageETCtoMTC>()
                        .WithIdentity("LaneImage", "ETC->MTC")
                        .Build();
                    ITrigger trigImage = TriggerBuilder.Create()
                        .WithSimpleSchedule(s => s.WithIntervalInMinutes(imageJob.IntervalSynchrozation).RepeatForever())
                        .StartNow()
                        .Build();
                    scheduler.ScheduleJob(jobImage, trigImage);
                }
                ...
                //    .WithSimpleSchedule(s => s.WithIntervalInMinutes(time).RepeatForever())
                //    .StartNow()
                //    .Build();
