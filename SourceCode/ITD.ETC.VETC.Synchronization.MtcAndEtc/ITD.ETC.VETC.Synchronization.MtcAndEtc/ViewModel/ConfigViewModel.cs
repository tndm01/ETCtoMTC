...
using Quartz.Util;
using EnumAndConst = ITD.ETC.VETC.Synchonization.Controller.EnumAndConst;
...
namespace ITD.ETC.VETC.Synchronization.MtcAndEtc.ViewModel
{
    public class ConfigViewModel : ViewModelBase
    {
        ...
        public ConfigViewModel()
        {
            ...
            //// Get all Job list
            //ObservableCollection<JobInformationModel> list = getAllJobList();

            
            //if (list.Count > 0)
            //{
            //    // Assign Etc List
            //   var etc =  list.Where(j => j.JobGroupType == Model.EnumAndConst.JobGoupType.Etc);
            //    foreach (var job in etc)
            //    {
            //        EtcJobList.Add(job);
            //    }

            //    // Assign Mtc List
            //    var mtc = list.Where(j => j.JobGroupType == Model.EnumAndConst.JobGoupType.Mtc);
            //    foreach (var job in mtc)
            //    {
            //        MtcJobList.Add(job);
            //    }
            //}
        ...
        private ObservableCollection<JobInformationModel> getAllJobList()
        {
            ObservableCollection<JobInformationModel> listJob = new ObservableCollection<JobInformationModel>();
            try
            {
                string connectionString = DataBaseHelper.GetConnectionString(_databaseServerName,
                    _databaseName, _databaseUser, _databaseUserPassword,
                    _databaseTimeout.ToString());
                ...
                if (isDataConnected)
                {
                    ...
                    cmd.CommandText = EnumAndConst.STORE_GET_JOBLIST;
                    ...
                    if (dt != null)
                    {
                        ...
                        {
                            JobInformationModel job = new JobInformationModel();
                            job.JobName = row["JobName"].ToString();
                            job.SourcePath = row["SubFolder"].ToString();
                            if(!String.IsNullOrEmpty(row["IsRun"].ToString()))
                            {
                                job.IsRun = row["IsRun"].ToString() == "1";
                            }

                            if (!String.IsNullOrEmpty(row["Interval"].ToString()))
                            {
                                job.IntervalSynchrozation = int.Parse(row["Interval"].ToString());
                            }

                            if (!String.IsNullOrEmpty(row["JobGroup"].ToString()))
                            {
                                int type = int.Parse(row["JobGroup"].ToString());
                                job.JobGroupType = (Model.EnumAndConst.JobGoupType) type;
                            }
